...
def test_token_count_follows_word_formula():
...
def test_token_count_capped_by_max_tokens():
...
def test_token_count_floor_is_one_for_empty_prompt():
...
def test_batch_size_is_one_for_sequential_requests():
...
def test_served_ms_is_non_negative_float():
    resp = client.post("/infer", json={"prompt": "test"})
    body = resp.json()
...
def test_served_ms_is_rounded_to_two_decimal_places():
    resp = client.post("/infer", json={"prompt": "test"})
